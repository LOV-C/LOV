...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("LOV://"))
    {
        uri.replace(0, 12, "LOV:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LOV.lnk";
}
...
{
    return GetAutostartDir() / "LOV.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("LOV-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  LOV-qt [" + tr("command-line options") + "]                     " + "\n";
